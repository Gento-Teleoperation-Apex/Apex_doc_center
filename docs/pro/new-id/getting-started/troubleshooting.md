...
| 打包姿态直接 Home 可能碰撞立柱 | 禁止继续 Home；先启动 Robot 并完成 Ready，开启拖动模式，手动将双臂移到标准零位姿态后关闭拖动模式 |
| 拖动模式无法开启 | 检查 Robot 是否运行、是否已点击 Start Robot，以及 `/tj/control/set_drag` 是否返回成功 |
| Home 过程中腕部相机接近立柱 | 立即停止并按需急停，重新确认是否已退出打包姿态、关闭拖动模式并留出安全距离 |
