...
当前源码核对快照为 `v1.0.6.95g-beta-10-g14cfc0d`，不等同于客户正式安装包版本。控制器端、Teleop 遥操服务、Apex 前端和 Pico 客户端必须使用同一交付批次的配套版本。升级后依次验证 Robot、3D 姿态、相机、Pico 连接和 Home，再按机型验证 Skye 的双臂/BODY/LIFT 或 Luna 的双臂/BODY 小幅动作。
